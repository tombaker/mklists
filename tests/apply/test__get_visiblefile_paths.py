...
from mklists.apply import _find_visiblefile_paths


def test_find_visiblefile_paths_ignoring_directories(tmp_path):
    ...
    Path("baz").mkdir()
    print(os.listdir())
    assert _find_visiblefile_paths() == [
...
def test_ignore_hidden_dotfiles(tmp_path):
    ...
    Path(".bar").write_text("bar stuff")
    Path("foo").write_text("foo stuff")
    assert _find_visiblefile_paths() == [str(Path("foo").resolve())]
