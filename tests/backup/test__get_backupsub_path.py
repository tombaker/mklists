...
from mklists.backup import _get_backupsub_path


def test_get_backupsub_path(tmp_path):
    ...
    cwd.mkdir()
    os.chdir(cwd)
    actual = _get_backupsub_path(backupdir_name=backdir, now=datestr)
...
def test_get_backupsub_path_given_datadir(tmp_path):
    ...
    actual = _get_backupsub_path(datadir=cwd, backupdir_name=backdir, now=datestr)
    expected = Path(tmp_path) / backdir / shortname_expected / datestr
    assert actual == expected


def test_get_backupsub_path_given_datadir_with_slash(tmp_path):
