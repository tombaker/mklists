...
def test_make_backup_config_given_relative_return_relative():
    ...
    config_rootdir = Path("/datatree/root")
...
def test_make_backup_config_null_backup_rootdir_returns_disabled(tmp_path):
    """backup_rootdir configured as null returns a disabled BackupConfig."""
    config_dict = {
        "backup": {
            "backup_rootdir": None,
            "backup_depth": 3,
    ...
    backup_cfg = _make_backup_config(config_dict=config_dict, config_rootdir=tmp_path)

    assert backup_cfg.backup_rootdir is None
    assert backup_cfg.backup_depth == 3
