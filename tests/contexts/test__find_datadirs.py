...
def test_find_datadirs_sorted(tmp_path):
    """Yields directories in sort order."""
    d1 = tmp_path / "b"
    d2 = tmp_path / "a"
...
def test_finds_datadirs_even_in_hidden_directories(tmp_path):
    ...
    FYI: In practice, it should never happen that a user creates a hidden 
    data directory. It seems excessive for `_find_datadirs` to explicitly
    guard against this.
    """
    d = tmp_path / ".a"
    d.mkdir(parents=True)
